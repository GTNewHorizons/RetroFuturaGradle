...
public final class ContextualPatch {
        ...
        try {
            ...
            for (SinglePatch patch : patches) {
                try {
                    ...
                    report.add(
                            new PatchReport(patch.targetPath, patch.binary, PatchStatus.Failure, e, new ArrayList<>()));
                }
            }
            return report;
        } finally {
    ...
    private void init() throws IOException {
        if (patchString != null) {
            // Just read the string as is, without trying to read the magic/encoding as the string shuldn't need
            // encoding!
     ...
     */
    private HunkReport applyHunk(List<String> target, Hunk hunk, int idx, boolean dryRun, int fuzz, int hunkID)
            throws PatchException {
        int startIdx = idx;
